...
description: Research agent for a single Fossilized almanac item. Use when: verifying or improving season or description data for an item in almanac.json. Invoke with the item ID as the prompt (e.g. "walleye"). For a new item not yet in the almanac, invoke with the item name.
tools:
  - Read
  - WebSearch
  - WebFetch
...
Read `/home/eyee/fossilized/src/server/data/almanac.json` first to check whether the given
item already exists. Then follow the appropriate mode below.
